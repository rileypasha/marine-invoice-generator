...
import { ChevronDown, Users, MapPin, Briefcase, Hash, ArrowUpDown, ArrowUp, ArrowDown, Filter } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
function GroupMenu({ activeGroupBy, onGroupByChange }: GroupMenuProps) {
  const groupOptions = [
    ...
    { key: 'company' as ContactGroupBy, label: 'Company', icon: <Briefcase className="h-4 w-4" /> },
    { key: 'city' as ContactGroupBy, label: 'City', icon: <MapPin className="h-4 w-4" /> },
    { key: 'role' as ContactGroupBy, label: 'Role', icon: <Users className="h-4 w-4" /> },
  ];
...
function FilterMenu({ activeFilters, onFiltersChange }: FilterMenuProps) {
  ...
  return (
    ...
    >
      ...
      <DropdownMenuCheckboxItem
        checked={activeFilters.hasEmail === true}
