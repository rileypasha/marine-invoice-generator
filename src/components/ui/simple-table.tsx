...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "sticky top-0 z-20 h-9 px-4 text-left align-middle text-[11px] font-semibold uppercase tracking-wider text-slate-400 bg-white border-b border-slate-200 [&:has([role=checkbox])]:pr-0",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <td
    ref={ref}
    className={cn("px-4 py-2 align-middle text-slate-700 [&:has([role=checkbox])]:pr-0", className)}
    {...props}
  />
))
